...
export default function SubmitJob() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await submitJob({
        map_sql: mapSQL || undefined,
        reduce_sql: reduceSQL || undefined,
        input_files: files,
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-white mb-6">Submit SQL Job</h1>
      <form
      ...
      >
        <div>
          ...
          <textarea
            ...
            rows={4}
            placeholder="SELECT key, value FROM input_table WHERE ..."
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            placeholder="SELECT key, SUM(value) FROM grouped GROUP BY key"
            className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white font-mono placeholder-gray-500 focus:outline-none focus:border-emerald-500 resize-y"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1.5">
            Input Files <span className="text-gray-500">(comma separated)</span>
          </label>
          <input
            value={inputFiles}
            onChange={(e) => setInputFiles(e.target.value)}
            placeholder="/data/sales.csv, /data/users.csv"
