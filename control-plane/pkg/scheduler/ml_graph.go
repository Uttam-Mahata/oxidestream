...
func (s *Scheduler) SubmitLinearRegressionJob(ctx context.Context, inputFiles []string, numPartitions int32, outputDir string, iterations int) error {
	log.Printf("Starting Distributed Linear Regression Job (OLS closed-form) with %d iteration(s) on inputs: %v", iterations, inputFiles)

	mapSQL := "SELECT * FROM input -- linear_regression"
	reduceSQL := "SELECT * FROM input -- linear_regression_reduce"

	for iter := 1; iter <= iterations; iter++ {
		log.Printf("Linear Regression: Iteration %d/%d...", iter, iterations)

		job, err := s.SubmitMapReduceJob(mapSQL, reduceSQL, inputFiles, numPartitions, outputDir)
		if err != nil {
			return fmt.Errorf("failed to submit Linear Regression iteration %d: %w", iter, err)
		}

		status, err := s.WaitForJob(job.JobID)
		if err != nil || status != "COMPLETED" {
			return fmt.Errorf("linear regression iteration %d failed with status %s: %v", iter, status, err)
		}

		log.Printf("Linear Regression: Iteration %d complete.", iter)
	}
...
func (s *Scheduler) SubmitPageRankJob(ctx context.Context, inputFiles []string, numPartitions int32, outputDir string, iterations int) error {
	...
	for iter := 1; iter <= iterations; iter++ {
		...
		mapSQL := "SELECT rank / out_degree AS rank_contrib, target FROM input"
		// Reduce Stage: Aggregate ranks per target node
		reduceSQL := "SELECT target, 0.15 + 0.85 * SUM(rank_contrib) AS rank FROM input GROUP BY target"
